...
class AgentsUtils:
    ...
    def get_agents_model(
        ...
        type = type or os.getenv("EBS_LLM_TYPE", "chat.completions")
        model = model or os.getenv("EBS_LLM_MODEL")
        if type == "litellm":
        ...
        base_url = base_url or os.getenv("EBS_LLM_BASE_URL")
        api_key = api_key or os.getenv("EBS_LLM_API_KEY")
        timeout = timeout or float(os.getenv("EBS_LLM_TIMEOUT", "600"))
        if not api_key or not base_url:
            raise ValueError("EBS_LLM_API_KEY and EBS_LLM_BASE_URL must be set")
        openai_client = AsyncOpenAI(
            api_key=api_key,
            base_url=base_url,
            timeout=httpx.Timeout(timeout=timeout, connect=min(timeout, 30.0)),
        )
    ...
    def get_trajectory_from_agent_result(agent_result: RunResult, agent_name: str = None) -> dict:
        ...
        trajectory = [dict(message) for message in ChatCompletionConverter.items_to_messages(agent_result.to_input_list())]
        usage_list = []
        for model_response in getattr(agent_result, "raw_responses", []):
            usage = getattr(model_response, "usage", None)
            if usage is not None:
                usage_list.append(asdict(usage))
        usage_idx = 0
        for message in trajectory:
            if message.get("role") == "assistant" and usage_idx < len(usage_list):
                curr_usage = usage_list[usage_idx]
                message["usage"] = {
                    "input_tokens": curr_usage.get("input_tokens"),
                    "output_tokens": curr_usage.get("output_tokens"),
                    "total_tokens": curr_usage.get("total_tokens"),
                    "requests": curr_usage.get("requests"),
                }
                usage_idx += 1
        return {
            "agent": agent_name,
