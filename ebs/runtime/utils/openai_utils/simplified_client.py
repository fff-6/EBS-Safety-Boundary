...
class SimplifiedAsyncOpenAI(AsyncOpenAI):
    ...
    def __init__(
        ...
        **kwargs: dict,
        ...
        super().__init__(
            api_key=api_key or os.getenv("EBS_LLM_API_KEY") or "xxx", base_url=base_url or os.getenv("EBS_LLM_BASE_URL")
        )
        self.type = type or os.getenv("EBS_LLM_TYPE", "chat.completions")
    ...
    def _process_kwargs(self, kwargs: dict) -> dict:
        ...
        for k, v in kwargs.items():
            if k in self.type_create_params:
                default_config[k] = v
        default_config["model"] = default_config.get("model", os.getenv("EBS_LLM_MODEL"))
