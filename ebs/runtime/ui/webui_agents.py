...
from ebs.runtime.agents.orchestra_agent import OrchestraAgent
from ebs.runtime.agents.simple_agent import SimpleAgent
from ebs.runtime.config import AgentConfig
from ebs.runtime.config.loader import ConfigLoader
from ebs.runtime.meta.simple_agent_generator import SimpleAgentGeneratedEvent, SimpleAgentGenerator
...
class WebUIAgents:
    def __init__(self, default_config: str):
        ...
        # hack
        with resources.as_file(resources.files("ebs_agent_ui.static").joinpath("index.html")) as static_dir:
            self.static_path = str(static_dir).replace("index.html", "")

    def make_app(self, autoload: bool | None = None) -> tornado.web.Application:
        if autoload is None:
            autoload = EnvUtils.get_env("EBS_WEBUI_AUTOLOAD", "false") == "true"
...
if __name__ == "__main__":
    webui = WebUIAgents(default_config="agents/simple/base")
    webui.launch()
